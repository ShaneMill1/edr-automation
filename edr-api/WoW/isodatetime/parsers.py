...
class TimeIntervalParser(object):
    ...
    Callable (via self.parse method) with an ISO 8601-compliant
    recurrence pattern - this returns a TimeInterval instance.

    """

    RECURRENCE_REGEXES = [
        re.compile(r"(?P<start>[^P][^/]*)/(?P<end>[^P].*)$")]
    ...
    def parse(self, expression):
        """Parse a recurrence string into a TimeInterval instance."""
        for regex in self.RECURRENCE_REGEXES:
            result = regex.search(expression)
            if not result:
                continue
        ...
        raise ISO8601SyntaxError("interval", expression)
